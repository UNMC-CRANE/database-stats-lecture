...
**Database:** A collection of related tables, each table with its own rows and columns A conceptual entity (Lab Order, Lab Result, Demographics) is represented by one table ***Attributes*** (lab unit or lab value) are represented by one column

Columns contain similar types of information (date and time or name) containing only one type of data (integer, varchar, binary) Rows represent one instance of a table entry, such as a lab order or lab result or a single patient for the Demographic table
